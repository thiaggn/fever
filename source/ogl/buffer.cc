...
gl_vertex_buffer::gl_vertex_buffer() : handle(0), acc_offset(0) {
    glGenBuffers(1, &handle);
    glBindBuffer(GL_ARRAY_BUFFER, handle);
}

gl_vertex_buffer::gl_vertex_buffer(const void *data, llint size_bytes): handle(0), acc_offset(0) {
    glGenBuffers(1, &handle);
    glBindBuffer(GL_ARRAY_BUFFER, handle);
...
void gl_vertex_buffer::location(int location, int size, gl_data_type d_type, gl_bool_value normalized) {
...
gl_index_buffer::gl_index_buffer(const void *data, llint size_bytes): handle(0) {
    glGenBuffers(1, &handle);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, handle);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, size_bytes, data, gl_buffer_usage::static_draw);
}
